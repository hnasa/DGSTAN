...
def FFT_for_Period(x, k=2):
    # [B, T, C]
    if isinstance(x, np.ndarray):  # 判断x是否为numpy数组
        x = torch.from_numpy(x)  # 将numpy数组转换为torch张量
    ...
    abs_xf = abs(xf).mean(-1)[:, top_list]  # 获取傅里叶变换的幅度
    abs_xf = abs_xf.numpy()  # 将torch张量转换为numpy数组
    return period, abs_xf
...
def generate_data(args: argparse.Namespace):
    ...
    add_time_of_tp2 = args.tp2
    add_time_of_tp3 = args.tp3
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tp2", type=bool, default=TP2,
                        help="Add feature period2.")
    parser.add_argument("--tp3", type=bool, default=TP3,
                        help="Add feature period3.")
